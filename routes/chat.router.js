...
router.get('/messages/:chat_id', async (req, res) => {
    try {
        const messages_query = 'SELECT message.*, user.user_name, user.profile_image\
                                FROM message\
                                INNER JOIN user\
                                ON message.user_id = user.user_id AND message.chat_id = ?'
    ...
    async (req, res) => {
        try {
            ...
            for (let i = 0; i < media_length; i++){
                console.log(message_media[i]);
                const data = await sharp(message_media[i].buffer).toFormat('webp').webp({ lossless: false, quality: 50 }).toBuffer()
                await database.query(create_message_media, [null, new_msg.insertId, data, 'image/webp'])
            }
